...
def quicklook(input_file_path: str) -> None:
    ...
    hist.fill(dr_abs, dr / grid.pitch)
    hist_mean, hist_sigma = hist.project_statistics()
    plt.figure("Reconstructed vs true distance from pixel center")
    plt.plot(hist_mean.bin_centers(), hist_mean.content.flatten(), 'o', label='Mean')
    input_file.close()
    plt.show()


class ResolutionDefaults:
    """Default parameters for the resolution task.
...
def resolution(input_file_path: str) -> None:
    """Calculate the distribution of the distance between reconstructed and true position
    and calculate its mean, standard deviation and FWHM.
    ...
    fwhm : float
        The FWHM of the distance distribution.
    """
    name, args = current_call()
    logger.info(f"Running {__name__}.{name} with arguments {args}...")
    input_file = ReconInputFile(input_file_path)
    header = input_file.digi_header
    # Get the reconstructed and true positions
    x_mc = input_file.mc_column("absx")
    y_mc = input_file.mc_column("absy")
    x = input_file.column("posx")
    y = input_file.column("posy")
    # Calculate the distance between reconstructed and true position
    dr = np.sqrt((x - x_mc)**2 + (y - y_mc)**2) / header["pitch"]
    # Create the histogram of the distance
    edges = np.linspace(0., 1., 101)
    hist = Histogram1d(edges, xlabel=r"$|\vec{r} - \vec{r}_{mc}|$ / pitch")
    hist.fill(dr)
    mean, stddev = hist.binned_statistics()
    # Calculate the FWHM if possible
    try:
        fwhm = hist.fwhm()
    except ValueError:
        fwhm = np.nan
