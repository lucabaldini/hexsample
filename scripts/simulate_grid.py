...
N_NEIGHBORS = np.arange(0,7)
# Zero-suppression threshold, expressed in units of enc.
SIGMA_THRESHOLD = np.linspace(0, 5, 50)
...
for thickness in THICKNESS:
    for noise in NOISE:
        for pitch in PITCH:
            ...
            # ... and reconstruct.
            threshold = 52
            suffix = f'recon_nn2_thr{threshold:.0f}'
...
for thickness in THICKNESS:
    for pitch in PITCH:
        # Simulate...
        file_name = f'sim_{1.e4 * thickness:.0f}um_0enc_{1e4 * pitch:.0f}pitch.h5'
        file_path = HEXSAMPLE_DATA / file_name
        kwargs = dict(outfile=file_path, thickness=thickness, noise=0, pitch=pitch)
        file_path = hxsim(numevents=NUM_EVENTS, **kwargs)
        # ... and reconstruct.
        threshold = 0
        suffix = f'recon_nn2_thr{threshold:.0f}'
...
for sigma_thr in SIGMA_THRESHOLD:
