...
def analyze_grid(thickness : np.array, enc : np.array, **kwargs) -> None:
    ...
    for thick_idx, thick in np.ndenumerate(thickness):
        for e_idx, e in np.ndenumerate(enc):
            ...
            if correct_1px_ratio is True:
                onepx_evts_ratio[thick_idx][e_idx] = energy_hist_1px.content.sum() / \
                    energy_hist.content.sum()
            recon_file.close()
    ...
    # Plotting the overlapped heatmaps and customizing them.
    _, ax = double_heatmap(enc, thickness, mean_shift_ka_1px.flatten(),\
    ...
    _, ax2 = double_heatmap(enc, thickness, energy_res_ka_1px.flatten(),
                              energy_res_kb_1px.flatten())
