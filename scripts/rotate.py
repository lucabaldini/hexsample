...
def rotate():
    ...
    x_eta = recon_file_eta.column("posx") * 10000  # Convert to microns
    y_eta = recon_file_eta.column("posy") * 10000  # Convert to microns

    recon_file_centroid = ReconInputFile(file_path_centroid)
    x_centroid = recon_file_centroid.column("posx") * 10000  # Convert to microns
    y_centroid = recon_file_centroid.column("posy") * 10000  # Convert to microns

    xedges = np.arange(min(x_eta), max(x_eta), 10)
    yedges = np.arange(min(y_eta)-10, max(y_eta)-10, 10)

    plt.hist2d(x_eta, y_eta, bins=(xedges, yedges))
