...
DATA_INPUT_FILE_PATH = Path("/home/augusto/asix/hdf/020_0006531_data_all.h5")
...
def reconstruct_calibrated_data():
    ...
    # matrix = CalibrationMatrixNoise(num_cols, num_rows)
    # gain_matrix = CalibrationMatrixGain(num_cols, num_rows, 1e4, 16)
    ...
    zero_sup_threshold = 16 / 0.082
    num_neighbors = 6
    clustering = ClusteringNN(readout, zero_sup_threshold, num_neighbors)


    energy_ar = []
    # noise_matrix = CalibrationMatrixNoise.from_hdf5("/home/augusto/asix/test_noise_matrix.h5")
    # noise_value = noise_matrix.value
    gain_matrix = CalibrationMatrixGain.from_hdf5("/home/augusto/asix/test_gain_matrix.h5")
    gain_value = gain_matrix.value
    for i, event in tqdm(enumerate(input_file)):
        slice_row, slice_col = event.roi.readout_slice()
        pha = event.pha.astype(float)
        try:
            pha /= gain_value[slice_row, slice_col]
        except ValueError as e:
            logger.warning(f"Error while applying gain correction to event {event.trigger_id}: {e}", exc_info=True)
            continue
        event.pha = pha.astype(int)
        try:
            cluster = clustering.run(event)
            energy_ar.append(cluster.pulse_height())
        except IndexError as e:
            continue
        except ZeroDivisionError as e:
            continue

    energy = np.array(energy_ar) * 3.6
    plt.hist(energy, bins=np.arange(2500., 15000., 100))
    input_file.close()
    plt.show()
...
def compare():
    ...
    adc_counts_data = np.round(data_recon_file.column("energy") / 0.08).astype(int)
    pha_hist_data = Histogram1d(np.arange(2500, 15000, 100), xlabel="ADC")
